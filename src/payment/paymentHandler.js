...
export const createPayment = async (user, amount, items, integration_id) => {
  ...
  let paymentToken = await generatePaymentKey(
    token,
    orderId,
    user,
    amount,
...
export const getPaymentKeyCreditCard = async (
  user,
  amount,
  items,
  integration_id
) => {
  let { orderId, token } = await createPayment(
    user,
    amount,
    items,
    integration_id
...
export const createPaymentLinkMultiMethods = async (
  amount,
  integration_ids,
  items,
  data
  ...
  try {
    ...
    console.log("ERROR 2. PAYMOB: ", err);
  }
};
